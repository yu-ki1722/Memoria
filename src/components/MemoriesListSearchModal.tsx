...
type MemoriesListSearchModalProps = {
  isOpen: boolean;
  onClose: () => void;
  ...
  setSelectedEmotions: (e: string[]) => void;
  selectedTags: string[];
  ...
  setPrefectureInput: (p: string) => void;
  cityInput: string;
  setCityInput: (c: string) => void;
  placeInput: string;
  setPlaceInput: (p: string) => void;
  onReset: () => void;
};

export default function MemoriesListSearchModal({
  isOpen,
  onClose,
  onFilterResults,
  supabase,
  ...
  }, [isOpen, fetchTags, supabase]);

  const toggleEmotion = (emoji: string) => {
    setSelectedEmotions(
      selectedEmotions.includes(emoji)
        ? selectedEmotions.filter((e) => e !== emoji)
        : [...selectedEmotions, emoji]
    );
  };
