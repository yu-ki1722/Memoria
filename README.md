# Memoria（メモリア）
**人と記憶、そして場所を繋ぐ、スマホ用思い出記録マップwebアプリケーション**
...
ひとつの「思い出」として地図上に記録できる 思い出記録マップWebアプリケーション(スマホ専用)です。
...
画像アップロードではファイル名の sanitizing（無効文字削除）やストレージパスの管理を徹底し、不正なパス操作が起こらないようにしています。  
